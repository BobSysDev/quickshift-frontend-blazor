...
public class SimpleAuthProvider : AuthenticationStateProvider
{
        ...
        GetAuthenticationStateAsync()
    {
        ...
        SimpleEmployeeDTO employeeDto =
            JsonSerializer.Deserialize<SimpleEmployeeDTO>(userAsJson)!;
        ...
        List<Claim> claims = new List<Claim>()
        {
            ...
            new Claim(ClaimTypes.NameIdentifier,
                employeeDto.WorkingNumber.ToString()),
    ...
    public async Task Register(string firstName, string lastName,
        int workingNumber, string email, string password, bool isManager)
    {
        ...
        HttpResponseMessage response =
            await httpClient.PostAsJsonAsync("/Auth/register", newEmployee);
        ...
        SimpleEmployeeDTO? simpleEmployeeDto =
            JsonSerializer.Deserialize<SimpleEmployeeDTO>(content,
        ...
        List<Claim> claims = new List<Claim>()
        {
            ...
            new Claim(ClaimTypes.NameIdentifier,
                simpleEmployeeDto.WorkingNumber.ToString()),
    ...
    public async Task<string> RegisterSomeoneElse(string firstName,
        string lastName, int workingNumber, string email, string password,
        bool isManager)
    {
        var newEmployee = new NewEmployeeDTO
        {
    ...
    {
        HttpResponseMessage response =
            await httpClient.PostAsJsonAsync("/Auth/authenticate",
                new AuthEmployeeDTO()
                    { Password = password, WorkingNumber = workingNumber });
        string content = await response.Content.ReadAsStringAsync();
        if (!response.IsSuccessStatusCode)
        {
            throw new Exception(content);
